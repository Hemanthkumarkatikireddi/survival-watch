...
// ============================================================
//
// Last valid GPS location stored in ESP32 RTC memory.
//
// This cache is useful when:
//     - GPS is OFF
//     - GPS does not get a fix
//     - watch restarts
//
// IMPORTANT:
// RTC cache is NOT permanent storage.
// Complete power loss can clear it.
//

RTC_NOINIT_ATTR GPSCache lastGPSCache;

// ============================================================
// VALIDATE GPS CACHE
// ============================================================

static bool isGPSCacheValid()
{
    if (
        lastGPSCache.magic !=
        GPS_CACHE_MAGIC
    ...
    if (!lastGPSCache.valid)
    {
        return false;
    }

    if (
        lastGPSCache.latitude < -90.0 ||
        lastGPSCache.latitude > 90.0
    )
    {
        return false;
...
)
{
    ...
    lastGPSCache.latitude =
        latitude;

    lastGPSCache.longitude =
        longitude;
...
// ============================================================
// CHECK VALID GPS CACHE
// ============================================================

bool hasValidGPSCache()
{
    return isGPSCacheValid();
}
...
bool restoreGPSCache()
{
    ...
    {
        Serial.println(
            "GPS CACHE: No valid cached location"
        );
