...
static DateTime findTithiStart(DateTime center)
{
    ...
    for (int i = 0; i < 36; i++)
    {
        ...
        // IMPORTANT:
        // Allow FreeRTOS IDLE task / watchdog to run.
        vTaskDelay(
            pdMS_TO_TICKS(1)
        );
    }
    ...
    for (int i = 0; i < 30; i++)
    {
        ...
        // IMPORTANT:
        // Prevent CPU starvation during the
        // astronomical binary search.
        vTaskDelay(
            pdMS_TO_TICKS(1)
        );
    }
...
NakshatraInfo calculateNakshatra(DateTime dateTime)
{
    ...
    // --------------------------------------------------------
    // Next Nakshatra
    // Current Nakshatra ends -> next Nakshatra starts
    // --------------------------------------------------------

    result.nextNumber =
        result.number + 1;

    // After Revati → Ashwini
    if (result.nextNumber > 27)
    {
        result.nextNumber = 1;
    }

    // --------------------------------------------------------
    // Next Nakshatra starts exactly when
    // current Nakshatra ends
    // --------------------------------------------------------
