...
// ============================================================
// TEST: MASA + PANCHANGA RUTU + ASTRONOMICAL SEASON
// ============================================================
...
)
{
    ...
    currentLongitude = longitude;
     ...
     // ----------------------------------------------------
    // IMPORTANT:
    // Synchronize GPS location with timezone.cpp
    //
    // getActiveTimezone() uses the GPS coordinates
    // stored inside timezone.cpp.
    // ----------------------------------------------------
    ...
    // ----------------------------------------------------
    // FIRST VALID GPS FIX
    // No previous full astronomy calculation location.
    // Therefore this fix performs FULL calculation.
    // ----------------------------------------------------
...
{
    // --------------------------------------------------------
    // RTC currently represents India time
    // --------------------------------------------------------
    ...
    // --------------------------------------------------------
    // Calculate local time DIRECTLY from the active timezone
    //
    ...
    DateTime localTime =
        ...
        TimeSpan(
            0,
            activeTimezone.utcOffsetMinutes,
            0,
            0
        );
    ...
    static String lastDebugTimezone = "";

    String debugTimezone =
        String(activeTimezone.countryName) +
        "|" +
        String(activeTimezone.timezoneId) +
        "|" +
        String(activeTimezone.utcOffsetMinutes);

    if (debugTimezone != lastDebugTimezone)
    {
        Serial.println();
        Serial.println(
            "========== HOME ACTIVE TIMEZONE =========="
        );
